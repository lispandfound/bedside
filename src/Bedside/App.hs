...
run display config = do
    ...
    let artJobs = [daily (artRefresh schedule) (job "art" (randomArt >>= pushWidget queue)) | toggleArt toggles]
    ...
    forM_ (mewoJobs <> bertJobs <> weatherJobs <> artJobs) forkIO
...
initialWidgets manager mewoVar toggles schedule latitude longitude =
    do
        mewo <-
        ...
        return $ [backgroundWidget | toggleBackground toggles] <> catMaybes [mewo, weatherW, bert, art]
...
randomArt :: IO Widget
randomArt = do
    idx <- randomRIO (0, length artWidgets)
    return $ artWidgets !! idx
